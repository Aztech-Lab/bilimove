...
- **2026-09-03**：根目录单一 `config.yaml` — 标题/简介/credit 模板 + 数字/标签/下载/转码，占位符傻瓜式自定义
- **2026-09-03**：手动添加视频 — `./run.sh --add` 交互输入 URL（绕过去重，用户指定必传）
- **2026-09-03**：失败视频自动重试 — 下载/上传失败的视频在重启/下一轮心跳自动重试
